...
public class RealtimeAuctionController {
	...
	public AuctionMessageDto<?> joinAuction(@DestinationVariable("productId") String productId,
			AuctionJoinRequestDto auctionJoinRequest) {
		try {
			...
			if (response.getStatus() == ResponseTypeEnum.SUCCESS)
				return new AuctionMessageDto<User>(response.getData(), AuctionMessageTypeEnum.NEW_USER_JOINED,
						auctionJoinRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
			return new AuctionMessageDto<String>(response.getData().toString(), AuctionMessageTypeEnum.AUCTION_ERROR,
					auctionJoinRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
		} catch (Exception e) {
			e.printStackTrace();
			return new AuctionMessageDto<String>(e.getMessage(), AuctionMessageTypeEnum.AUCTION_ERROR,
					auctionJoinRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
		}
	}
	...
	@SendTo("/rt-product/auction-updates/{productId}")
	@SuppressWarnings("unchecked")
	public AuctionMessageDto<?> bid(@DestinationVariable("productId") String productId,
			AuctionBidRequestDto auctionBidRequest) {
		try {
			Response<BidInformation> response = (Response<BidInformation>) auctionHandlerService.addBid(productId,
					auctionBidRequest);
			if (response.getStatus() == ResponseTypeEnum.SUCCESS)
				return new AuctionMessageDto<BidInformation>(response.getData(), AuctionMessageTypeEnum.BID_MADE,
						auctionBidRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
			return new AuctionMessageDto<String>(response.getData().toString(), AuctionMessageTypeEnum.AUCTION_ERROR,
					auctionBidRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
		} catch (Exception e) {
			e.printStackTrace();
	...
	@SendTo("/rt-product/auction-updates/{productId}")
	@SuppressWarnings("unchecked")
	public AuctionMessageDto<?> getDetails(@DestinationVariable("productId") String productId,
			AuctionDetailsRequestDto auctionDetailsRequest) {
		try {
			Response<Auction> response = (Response<Auction>) auctionHandlerService.getAuctionDetails(productId);
			if (response.getStatus() == ResponseTypeEnum.SUCCESS)
				return new AuctionMessageDto<Auction>(response.getData(), AuctionMessageTypeEnum.DETAILS_SHARED,
						auctionDetailsRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
			return new AuctionMessageDto<String>("Some error occurred", AuctionMessageTypeEnum.AUCTION_ERROR,
					auctionDetailsRequest.getUserId(), new Timestamp(System.currentTimeMillis()));
		} catch (Exception e) {
			e.printStackTrace();
			return new AuctionMessageDto<String>(e.getMessage(), AuctionMessageTypeEnum.AUCTION_ERROR,
